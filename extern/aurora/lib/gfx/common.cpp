...
void map_staging_buffer() {
  ...
  // Aurora calls g_instance.ProcessEvents() each begin_frame so progress is made.
  g_stagingBuffers[currentStagingBuffer].MapAsync(
      ...
      [](wgpu::MapAsyncStatus status, wgpu::StringView message) {
        ...
        if (status == wgpu::MapAsyncStatus::CallbackCancelled || status == wgpu::MapAsyncStatus::Aborted) {
          ...
          // emdawnwebgpu means the JS-side WebGPU Instance was reclaimed
          // (Chrome can do this on tab visibility changes, GPU process
          // restart, or driver reset). Once that happens every subsequent
          // MapAsync also fails — log the first occurrence loudly so the
          // user can correlate with their browser activity, but don't spam
          // the console for every retry.
          std::string_view msg{message};
          if (msg.find("Instance reference") != std::string_view::npos && !s_loggedInstanceLoss) {
            s_loggedInstanceLoss = true;
            Log.error("[FATAL] WebGPU Instance lost mid-session — Chrome reclaimed the GPU. "
                      "Reload the page to recover. (full message: {})", message);
          } else if (!s_loggedInstanceLoss) {
...
#ifdef __EMSCRIPTEN__
      // emdawnwebgpu fires MapAsync callbacks via the JS event loop; without an
      // Asyncify yield here the wasm thread spins forever, never letting the
      // callback transition Mapping → Mapped. ProcessEvents alone doesn't pump
      // pending JS tasks under wasm.
      emscripten_sleep(0);
#endif
    }
  }
