...
void after_submit() noexcept {
  ...
  for (const auto& pending : pendingMaps) {
    // AllowSpontaneous is a native-Dawn extension; standard browser WebGPU only
    // accepts WaitAnyOnly or AllowProcessEvents for MapAsync. Aurora drives
    // ProcessEvents from begin_frame so progress is made.
    pending.readbackBuffer.MapAsync(
        wgpu::MapMode::Read, 0, pending.byteSize, wgpu::CallbackMode::AllowProcessEvents,
