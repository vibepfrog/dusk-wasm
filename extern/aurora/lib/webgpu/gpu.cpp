...
// BackendBinding (utils::SetupWindowAndGetSurfaceDescriptor) is needed for
// every backend that creates a surface — including emscripten's browser
// WebGPU. Only the Dawn-native helper header is gated below.
...

  {
    ...
    deviceDescriptor.SetUncapturedErrorCallback(
        [](const wgpu::Device& device, wgpu::ErrorType type, wgpu::StringView message) {
          // Was FATAL — but a WebGPU validation error in any upstream pipeline
          // (RmlUi backend has a depth/stencil mismatch we haven't fixed yet,
          // imgui's wgpu backend may have similar) shouldn't immediately kill
          // the whole runtime. Log loudly; let the broken pipeline render
          // nothing rather than nuking the app.
          Log.error("WebGPU error {}: {}", underlying(type), message);
        });
    // emdawnwebgpu silently rejects AllowSpontaneous (same trap we hit on
     // MapAsync). Use AllowProcessEvents so the lost-device callback actually
     // fires from g_instance.ProcessEvents() in begin_frame's wait loop.
     // Without this, when Chrome reclaims the device (idle/visibility change/
     // GPU process restart) we get no notification — the next MapAsync just
     // fails with "external Instance reference no longer exists" repeatedly.
     // Loud logging includes the reason so we can tell driver-reset from
     // visibility-change from oom etc.
...
#ifdef WEBGPU_DAWN
    // SetLoggingCallback + wgpu::LoggingType are Dawn extensions; emdawnwebgpu
    // doesn't expose them. The browser routes WebGPU validation messages to
    // the JS console automatically, so we lose nothing visible by skipping it.
