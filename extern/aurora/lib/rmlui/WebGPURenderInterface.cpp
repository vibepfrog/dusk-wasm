...
void WebGPURenderInterface::BeginLayerPass(Rml::LayerHandle layer, wgpu::LoadOp loadOp, const char* label,
  ...
  const wgpu::RenderPassDepthStencilAttachment depthStencilAttachment{
      ...
      // The clip-mask view is Stencil8 (no depth aspect). The WebGPU spec says
      // depth* fields should not be set when there's no depth aspect, but the
      // C++ struct always has them. emdawnwebgpu reads the uninitialized
      // depthClearValue as NaN and rejects with "non-finite". Explicitly set
      // them to a finite value (0.0f) and Undefined load/store ops.
...
void WebGPURenderInterface::CreateDeviceObjects() {
  ...
  const auto create_pipeline = [&](const char* label, wgpu::CompareFunction compareFn,
    ...
    const wgpu::DepthStencilState depthStencilState{
        ...
        // ClipMaskStencilFormat is Stencil8 — no depth aspect. Native Dawn
        // tolerates leaving depthWriteEnabled defaulted; emdawnwebgpu reports
        // it as OptionalBool::True (the C++ wrapper's default) and validation
        // rejects with "depth aspect missing while depthWriteEnabled is true".
        // Pin both depth fields to safe explicit values.
        .depthWriteEnabled = wgpu::OptionalBool::False,
        .depthCompare = wgpu::CompareFunction::Undefined,
