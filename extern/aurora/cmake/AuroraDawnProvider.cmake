...
elseif (_aurora_dawn_provider STREQUAL "emscripten")
  # ── Emscripten: emdawnwebgpu port (browser's WebGPU API) ──
  # Aurora has no WebGPU surface backend on native — Dawn provides D3D12/Vulkan/Metal/etc.
  # underneath. In the browser, the surface IS the WebGPU API, so we link the emdawnwebgpu
  # port (shipped with emsdk) and gate every other DAWN_ENABLE_* off so aurora_core.cmake
  # doesn't try to compile MetalBinding.mm or pull in Vulkan loader symbols.
  if (NOT EMSCRIPTEN)
    message(FATAL_ERROR "AURORA_DAWN_PROVIDER=emscripten requires the Emscripten toolchain")
  endif ()
  message(STATUS "aurora: Using Dawn via emdawnwebgpu port (provider=emscripten)")
  ...
  add_library(aurora_dawn_emdawnwebgpu INTERFACE)
  # --use-port=emdawnwebgpu must be on both compile and link steps: the port adds
  # webgpu/webgpu_cpp.h to the include path (compile) and the JS glue + symbol
  # bindings to the wasm module (link).
  target_compile_options(aurora_dawn_emdawnwebgpu INTERFACE --use-port=emdawnwebgpu)
  target_link_options(aurora_dawn_emdawnwebgpu    INTERFACE --use-port=emdawnwebgpu)
  # Aurora's PreferredBackendOrder (lib/aurora.cpp:41) is gated on this define;
  # without it the WebGPU surface backend is invisible and initialize() bails.
  target_compile_definitions(aurora_dawn_emdawnwebgpu INTERFACE ENABLE_BACKEND_WEBGPU)

  add_library(dawn::webgpu_dawn ALIAS aurora_dawn_emdawnwebgpu)
  set(AURORA_DAWN_IS_SHARED FALSE PARENT_SCOPE)

else ()
  message(FATAL_ERROR "Invalid AURORA_DAWN_PROVIDER: ${AURORA_DAWN_PROVIDER} "
    "(must be auto, vendor, system, package, or emscripten)")
endif ()
