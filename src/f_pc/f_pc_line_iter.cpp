...
static int fpcLnIt_MethodCall(node_class* i_createTagArg, void* i_filterArg) {
    create_tag_class* i_createTag = (create_tag_class*)i_createTagArg;
    ...
    ret = cTgIt_MethodCall(i_createTagArg, i_filterArg);
...
void fpcLnIt_Queue(fpcLnIt_QueueFunc i_queueFunc) {
    ...
    cTrIt_Method(&g_fpcLn_Queue, fpcLnIt_MethodCall, &filter);
}
